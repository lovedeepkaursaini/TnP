...
//cout<<"mask : "<<mask<<endl;
//cout<<ele->pt() <<'\t'<< ele->phi() <<'\t'<< ele->eta()<<endl;
...
if (vtxs->size() > 0) {
 ...
 {
        reco::VertexRef vtx1(vtxs, i);
        std::cout<<i<<'\t'<<vtx1->position()<<'\t'<<vtx1->x()<<", isvalid: "<<vtx1->isValid()<<" , ndof: "<<vtx1->ndof()<<std::endl;
 }
}
...
unsigned int EgammaCutBasedEleId::TestWP(WorkingPoint workingPoint, const bool isEB, const float pt, const float eta,
    ...
    const bool vtxFitConversion, const unsigned int mHits, const double rho)
{
//cout<<"cut val: "<<isEB<<"\t"<<pt<<"\t"<<eta<<"\t id starts: "<<dEtaIn<<"\t"<<dPhiIn<<"\t"<<sigmaIEtaIEta<<"\t"<<hoe<<"\t"<<ooemoop<<"\t"<<d0vtx<<"\t"<<dzvtx<<"\t"<<vtxFitConversion<<"\t"<<mHits<<"\t  iso ch, em, nh, rho:   "<<iso_ch<<"\t"<<iso_em<<"\t"<<iso_nh<<"\t"<<rho<<"\t"<<endl;

//cout<<"\t  iso ch, em, nh, rho:   "<<iso_ch<<"\t"<<iso_em<<"\t"<<iso_nh<<"\t"<<rho<<"\t"<<endl;
    unsigned int mask = 0;
    ...
    else if (workingPoint == EgammaCutBasedEleId::MEDIUMIDonly) {
        cut_dEtaIn[0]        = 0.004; cut_dEtaIn[1]        = 0.007;
        cut_dPhiIn[0]        = 0.060; cut_dPhiIn[1]        = 0.030;
        cut_sigmaIEtaIEta[0] = 0.010; cut_sigmaIEtaIEta[1] = 0.030;
        cut_hoe[0]           = 0.120; cut_hoe[1]           = 0.100;
    ...
    else if (workingPoint == EgammaCutBasedEleId::MEDIUMISOonly) {
        if (pt >= 20.0) {
            cut_iso[0] = 0.150; cut_iso[1] = 0.150;
        }
        else {
...
//cout<< "isolation is: "<<iso <<" AEff, rhoPrime, iso_nh, iso_em (iso_n), iso_ch, iso:  "<< AEff<<'\t'<<rhoPrime<<'\t'<<iso_nh<<'\t'<<iso_em<<" ( "<<iso_n<<" ) "<<iso_ch<<'\t'<<iso<<endl;
    // test cuts
    if (fabs(dEtaIn) < cut_dEtaIn[idx])             mask |= DETAIN;
//    std::cout<<"0: "<<mask<<std::endl;
