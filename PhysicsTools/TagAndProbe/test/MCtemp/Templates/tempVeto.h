...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      if (!f) {
         ...
         f->cd("/hdfs/store/user/kaur/mrgdTnPTreeElec_DYJetsToLL_M-50_TuneZ2Star_8TeV-madgraphSummer12_DR53X-PU_S10_START53_V7A-v1_Sep5.root:/GsfElectronToIdVeto");
      }
...
Int_t tempVeto::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t tempVeto::LoadTree(Long64_t entry)
{
...
fChain->SetBranchAddress("probe_gsfEle_abseta", &probe_gsfEle_abseta, &b_probe_gsfEle_abseta);
   ...
   fChain->SetBranchAddress("probe_isWPVeto", &probe_isWPVeto, &b_probe_isWPVeto);
...
void tempVeto::Show(Long64_t entry)
{
...
Int_t tempVeto::Cut(Long64_t entry)
{
