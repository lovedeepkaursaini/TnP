...
#ifdef tempLoose_cxx
tempLoose::tempLoose(TTree *tree)
{
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      if (!f) {
         ...
         f->cd("/hdfs/store/user/kaur/mrgdTnPTreeElec_DYJetsToLL_M-50_TuneZ2Star_8TeV-madgraphSummer12_DR53X-PU_S10_START53_V7A-v1_Sep5.root:/GsfElectronToIdLoose");
      }
...
Int_t tempLoose::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t tempLoose::LoadTree(Long64_t entry)
{
...
void tempLoose::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("probe_isWPTight", &probe_isWPTight, &b_probe_isWPTight);
...
void tempLoose::Show(Long64_t entry)
{
...
Int_t tempLoose::Cut(Long64_t entry)
{
