...
void tempMedium::Loop()
{
   ...
   if (fChain == 0) return;
    ...
    TFile* file  = new TFile("mcTruemadgraph_Medium.root","RECREATE");
    TH1D* hCounter = new TH1D("hcounter","hcounter",4,0,4);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(probe_isWPMedium) key = key+"_Pass";
      else key = key+"_Fail";
...
//      (histos[key])->Fill(mass,PUweight);
      ...
      /*  Simple Test*/
	double goodEvent = 1.5;
	if(tag_dRTau >= 90 && probe_dRTau >= 90)
 	    goodEvent = 2.5;

	hCounter->Fill(goodEvent);
}
file->cd();
file->Write();
