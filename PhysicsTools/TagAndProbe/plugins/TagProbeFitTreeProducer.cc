...
//cout<<"pass hona  "<<pairs.size()<<endl;    // loop on them to fill the tree
    for (tnp::TagProbePairs::const_iterator it = pairs.begin(), ed = pairs.end(); it != ed; ++it) {
        ...
        float mcMass = 0.f;
        if (isMC_) {
//cout<<" ander pass hona  "<<endl;    // loop on them to fill the tree
            reco::GenParticleRef mtag = (*tagMatches)[it->tag], mprobe = (*probeMatches)[it->probe];
           //cout<<"checkMother(mtag)    "<<checkMother(mtag)<<"   checkMother(mprobe)   "<<checkMother(mprobe); 
