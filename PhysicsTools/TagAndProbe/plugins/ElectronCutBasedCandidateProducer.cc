...
//std::cout<<iso_ch<<"\t"<<iso_em<<"\t"<<iso_nh<<std::endl;
      ...
      bool medium     = EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::MEDIUM, myElectronRef,
                      conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rhoIso);
     bool mediumIDonly= EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::MEDIUMIDonly, myElectronRef,      
                      conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rhoIso);
     bool mediumISOonly= EgammaCutBasedEleId::PassWP(EgammaCutBasedEleId::MEDIUMISOonly, myElectronRef, 
                      conversions_h, beamSpot, vtx_h, iso_ch, iso_em, iso_nh, rhoIso);
...
//combrelisoCutEE_) ){
	...
	if(nam=="veto") pass = veto;
        ...
        else if(nam=="mediumIDonly")pass= mediumIDonly;
        else if(nam=="mediumISOonly")pass= mediumISOonly;
	else if(nam=="tight")pass= tight;
//cout<<"which ID   "<<pass<<endl;
	if (pass) {
