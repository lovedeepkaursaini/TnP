...
//   cout<<"arbitration  None" << pairs.size()<<  endl;
  // read from event
...
//   cout<<" before loop " << mother.numberOfDaughters() << '\t'<< mother.mass() <<'\t'<< pairs.size()<<  endl;
...
//   cout<<" in loop " << mother.numberOfDaughters() << '\t'<< mother.mass() <<'\t'<< pairs.size()<<  endl;
